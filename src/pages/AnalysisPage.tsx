...
export default function AnalysisPage() {
  const { videoId } = useParams(); const id = videoId ?? ''; const navigate = useNavigate(); const { status, isFallback, refresh } = useAnalysisProgress(id); const { retry, isRetrying, error: retryError } = useAnalysisRetry(); const [isCancelling, setIsCancelling] = useState(false); const [cancelError, setCancelError] = useState<string | null>(null)
  useEffect(() => {
  ...
  async function handleRetry() { const result = await retry(id); if (result) { await refresh() } }
