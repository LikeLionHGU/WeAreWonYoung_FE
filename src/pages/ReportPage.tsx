...
import { formatTime, formatSeconds, reportEventTitle, reportEventSpeech, reportEventKind } from '../utils/format'
...
export default function ReportPage() {
  ...
  const videoRef = useRef<HTMLVideoElement>(null); const selectedCardRef = useRef<HTMLElement>(null); const [selectedId, setSelectedId] = useState<string | null>(null); const [decisions, setDecisions] = useState<Record<string, ReviewAction | null>>({}); const [isPlaying, setIsPlaying] = useState(false); const [duration, setDuration] = useState(0); const [currentTime, setCurrentTime] = useState(0); const [filter, setFilter] = useState<ReportFilter>('ALL'); const [decisionError, setDecisionError] = useState<string | null>(null); const [savingEventId, setSavingEventId] = useState<string | null>(null); const [isCompleting, setIsCompleting] = useState(false)
  useEffect(() => {
  ...
  const orderedEvents = [...filteredEvents].sort((left, right) => left.startMs - right.startMs)
  ...
  return <main className="report-page">
    <div className="report-content">
      ...
      {filter !== 'ALL' && <section className="report-filter-results" aria-label={`${filterLabel} 검토 후보`}><div className="report-filter-results-head"><strong>{filterLabel} 검토 후보</strong><span>{orderedEvents.length}건</span></div><div className="report-filter-results-list">{orderedEvents.map(event => <button type="button" className={`report-filter-result ${event.id === selected.id ? 'is-current' : ''}`} key={event.id} onClick={() => selectEvent(event)}><span className="report-time-pill">{formatTime(event.startMs)}</span><span>{reportEventKind(event)}</span><strong>{reportEventTitle(event)}</strong><small>{reportEventKind(event)}</small>{decisionLabel(event) && <em>{decisionLabel(event)}</em>}</button>)}</div></section>}
      ...
      <section className="report-candidates"><div className="report-candidates-card">{orderedEvents.map(event => <button type="button" className={`report-candidate-row ${event.id === selected.id ? 'is-current' : ''}`} key={event.id} onClick={() => selectEvent(event)}><span className="report-time-pill">{formatTime(event.startMs)}</span><span>{reportEventKind(event)}</span><strong>{reportEventTitle(event)}</strong><small>{reportEventKind(event)}</small>{decisionLabel(event) && <em>{decisionLabel(event)}</em>}</button>)}</div></section><p className="report-footnote">최종 판단은 제작자가 합니다. 원본 영상은 분석 완료 후 24시간 안에 삭제됩니다.</p>
    </div>
