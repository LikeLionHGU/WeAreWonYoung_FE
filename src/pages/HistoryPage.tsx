...
export default function HistoryPage() {
  ...
  return <main className="history-page"><h1>검수 이력</h1><p className="history-intro">직접 업로드하고 검수를 시작한 영상만 이곳에 표시됩니다.</p><div className="history-filters" role="group" aria-label="상태 필터"><button type="button" aria-pressed={filter === 'ALL'} className={filter === 'ALL' ? 'is-selected' : ''} onClick={() => setFilter('ALL')}>전체 {items.length}</button><button type="button" aria-pressed={filter === 'COMPLETED'} className={filter === 'COMPLETED' ? 'is-selected' : ''} onClick={() => setFilter('COMPLETED')}>완료 {completedCount}</button><button type="button" aria-pressed={filter === 'FAILED'} className={filter === 'FAILED' ? 'is-selected' : ''} onClick={() => setFilter('FAILED')}>실패 {failedCount}</button></div><div className="history-card">{visibleItems.length > 0 ? visibleItems.map(item => <HistoryRow item={item} key={item.videoId} />) : <div className="history-empty"><p>이 상태의 검수 이력이 없습니다.</p></div>}</div></main>
}
