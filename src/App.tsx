...
function ReportPage() {
  ...
  function selectEvent(event: TimelineEvent) { setSelectedId(event.id); const nextTime = event.startMs / 1000; setCurrentTime(nextTime); if (videoRef.current) { videoRef.current.currentTime = nextTime; void videoRef.current.play().catch(() => undefined) } }
  ...
  return <main className="report-page">
    <div className="report-content">
      ...
      <section className="report-candidates"><div className="report-candidates-card">{reportEvents.map(event => <button type="button" className={`report-candidate-row ${event.id === selected.id ? 'is-current' : ''}`} key={event.id} onClick={() => selectEvent(event)}><span className="report-time-pill">{formatTime(event.startMs)}</span><span>{reportEventKind(event)}</span><strong>{reportEventTitle(event)}</strong><small>{reportEventEvidence(event)}</small>{decisionLabel(event) && <em>{decisionLabel(event)}</em>}</button>)}</div></section><p className="report-footnote">최종 판단은 제작자가 합니다. 원본 영상은 분석 완료 후 24시간 안에 삭제됩니다.</p>
    </div>
