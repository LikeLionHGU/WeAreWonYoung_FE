...
function LandingUploadCta({ compact = false }: { compact?: boolean }) {
  return <div className={`landing-upload-cta ${compact ? 'landing-upload-cta-compact' : ''}`}>
    <Link className="landing-upload-button" to="/upload"><span>영상 업로드하러가기</span><strong aria-hidden="true">→</strong></Link>
  </div>
}
...
export default function LandingPage() {
  return <main className="landing-page">
    <section className="landing-hero"><div className="landing-hero-main"><LandingKicker label="업로드 전 검수" /><h1>공개 전 영상에서, 다시 확인할 구간과 근거를 먼저 보여드립니다</h1><p className="landing-lead">발언, 사실 정보, 관련 맥락을 분석해 사람이 다시 확인할 지점을 타임라인으로 제시합니다.<br />판정하거나 고치지 않습니다. 최종 판단은 제작자가 합니다.</p><div className="landing-hero-bottom"><LandingUploadCta /></div><p className="landing-privacy">원본 영상은 분석 완료 후 24시간 내 삭제 · 모델 학습에 사용하지 않음</p><LandingSteps /></div></section>
    ...
    <section className="landing-section landing-cta"><div><h2>한 편으로 확인해 보세요</h2><p>10분 영상 기준 5분 이내에 리포트가 나옵니다.</p></div><LandingUploadCta compact /></section>
    <footer className="landing-footer"><div><strong>제품</strong><Link to="/upload">업로드</Link><Link to="/report">검수 리포트</Link><Link to="/history">검수 이력</Link></div><div><strong>정책</strong><button type="button" className="footer-link">데이터 보관</button><button type="button" className="footer-link">AI 출력 원칙</button><button type="button" className="footer-link">참조 데이터베이스 기준</button></div><div className="landing-footer-brand"><img className="landing-footer-logo" src={logoUrl} alt="OoPs!?" /><p>AI는 의도와 정치 성향, 위법 여부를 판정하지 않습니다.</p><small>© 2026 OoPs?!</small></div></footer>
  </main>
}
