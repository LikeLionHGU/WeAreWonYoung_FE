...
async function init() {
  ...
  try {
    ...
    state.videos = Object.fromEntries(
      Object.entries(parsed.videos ?? {}).map(([key, raw]) => {
        ...
        const record: VideoRecord = {
          ...
          failure:
            legacy.failure ??
            (legacy.errorCode
              ? { code: legacy.errorCode, message: legacy.message ?? '분석에 실패했습니다.' }
              : null),
        }
    ...
    await Promise.all(
      Object.values(state.videos).map(async record => {
...
function reviewSummary(events: TimelineEvent[]) {
  const count = (action: ReviewAction) =>
    events.filter(event => event.reviewAction === action).length
  const decided = events.filter(event => event.reviewAction !== null).length
  return {
    decided,
    remaining: events.length - decided,
    confirmed: count('CONFIRMED'),
    edited: count('EDITED'),
    hold: count('HOLD'),
    notUseful: count('NOT_USEFUL'),
  }
}
  ...
  const events: TimelineEvent[] = [
    {
      ...
      type: 'SPEECH',
      severity: 'HIGH',
      candidateType: 'SPEECH_REVIEW',
      title: '특정 세대를 단정하는 표현',
      text: '요즘 젊은 사람들은 다 책임감이 없는 것 같아요.',
      contextBefore: '제가 최근에 몇 명을 만나봤는데요.',
      contextAfter: '물론 제가 만난 사례만 보고 느낀 생각입니다.',
      riskTypes: ['GENERALIZATION'],
      reason: '특정 세대 전체를 하나의 특성으로 묶어 표현하고 있습니다.',
      frameUrl: frame('event-1'),
      references: [],
      reviewAction: null,
    },
    {
      ...
      type: 'SPEECH',
      severity: 'MEDIUM',
      candidateType: 'FACT_CHECK',
      title: '설립 연도 사실 확인',
      text: 'OO회사는 2019년에 설립됐습니다.',
      riskTypes: ['FACT_CHECK'],
      reason:
        '영상에서는 설립 연도를 2019년이라고 언급했지만 확인된 자료에서는 2020년으로 안내하고 있습니다.',
      frameUrl: frame('event-2'),
      references: [
        {
          title: 'OO회사 공식 소개',
          provider: 'OO회사',
          url: 'https://example.com/company',
          relevantContext: '발언의 설립 연도 확인에 사용',
        },
      ],
      reviewAction: null,
    },
    {
      ...
      title: '화면 수치 출처 확인',
      speechText: '',
      captionText: '2024년 국내 이용자 80%',
      reason: '화면에 구체적인 이용률 수치가 표시되어 있어 출처 확인이 필요합니다.',
      frameUrl: frame('event-3'),
      references: [
        {
          title: '2024 이용자 조사',
          provider: 'OO기관',
...
function normalizeReport(record: VideoRecord, report?: Partial<Report>): Report {
  ...
  const actions = new Map(
    report.events.map(event => [String(event.id), event.reviewAction ?? null]),
  )
  normalized.events.forEach(event => {
    event.reviewAction = actions.get(event.id) ?? null
  })
  normalized.reviewStatus =
    report.reviewStatus ??
    (normalized.events.some(event => event.reviewAction) ? 'IN_REVIEW' : 'NOT_STARTED')
...
function publish(record: VideoRecord) {
  ...
  for (const [socket, topics] of subscriptions) {
    for (const [topic, subscriptionId] of topics) {
      if (topic === destination && socket.readyState === socket.OPEN)
        socket.send(
...
async function advance(record: VideoRecord) {
  ...
  const timer = setInterval(async () => {
    ...
    if (!current || current.jobId !== record.jobId || current.status === 'CANCELLED') {
      clearInterval(timer)
      return
    }
    ...
    current.progress =
      current.stage === 'COMPLETED'
        ? 100
        : Math.min(95, Math.round(((index + 1) / stages.length) * 100))
    current.message =
      current.stage === 'COMPLETED' ? '검수 결과가 준비되었습니다.' : stageMessage(current.stage)
    current.updatedAt = new Date().toISOString()
...
function stageMessage(stage: Stage) {
  const messages: Partial<Record<Stage, string>> = {
    STT: '음성을 텍스트로 변환 중',
    TEXT_RISK: '발언 검토 후보 분석 중',
    SCENE_DETECTION: '사실 정보를 확인 중',
    OCR: '관련 맥락을 확인 중',
    MULTIMODAL: '검토 후보와 근거를 정리 중',
    FINALIZING: '검수 결과를 정리 중',
  }
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) =>
    cb(null, ['.mp4', '.mov', '.avi'].includes(path.extname(file.originalname).toLowerCase())),
})
...
app.post('/api/v1/videos', (req, res) => {
  upload.single('file')(req, res, async error => {
    ...
    if (error)
      return failure(res, 415, 'UNSUPPORTED_VIDEO_FORMAT', '지원하지 않는 영상 형식입니다.')
    ...
    const record: VideoRecord = {
      ...
      mimeType: mimeFor(req.file.originalname),
...
app.get('/api/v1/videos/history', (req, res) => {
  ...
  const all = Object.values(state.videos)
  ...
  const items = all
    ...
    .map(record => ({
      ...
      reviewStatus: record.report?.reviewStatus ?? 'NOT_STARTED',
      eventCount: record.report?.events.length ?? 0,
      editedCount:
        record.report?.events.filter(event => event.reviewAction === 'EDITED').length ?? 0,
...
app.get('/api/v1/videos/:id/report', (req, res) => {
  ...
  if (record.status !== 'COMPLETED' || !record.report)
    return failure(res, 409, 'ANALYSIS_NOT_COMPLETED', '분석이 아직 완료되지 않았습니다.')
...
app.put('/api/v1/videos/:id/review-actions/:eventId', async (req, res) => {
  ...
  if (!['CONFIRMED', 'EDITED', 'HOLD', 'NOT_USEFUL'].includes(action))
    return failure(res, 400, 'INVALID_REQUEST', '검수 결정 값이 올바르지 않습니다.')
...
app.post('/api/v1/videos/:id/review-completion', async (req, res) => {
  ...
  if (summary.remaining > 0)
    return failure(res, 409, 'REVIEW_INCOMPLETE', '결정하지 않은 검토 후보가 남아 있습니다.')
  ...
  success(res, 200, {
    videoId: record.videoId,
    reviewStatus: record.report.reviewStatus,
    reviewedAt: record.reviewedAt,
    summary: {
      total: record.report.events.length,
      decided: summary.decided,
      remaining: summary.remaining,
      confirmed: summary.confirmed,
      edited: summary.edited,
      hold: summary.hold,
      notUseful: summary.notUseful,
    },
  })
})
...
app.post('/api/v1/videos/:id/analysis/retry', async (req, res) => {
  ...
  if (record.status === 'PROCESSING' || record.status === 'PENDING')
    return failure(res, 409, 'ANALYSIS_IN_PROGRESS', '분석이 진행 중입니다.')
  if (record.status !== 'FAILED' && record.status !== 'CANCELLED')
    return failure(
      res,
      409,
      'INVALID_ANALYSIS_STATE',
      '실패하거나 취소된 분석만 다시 시작할 수 있습니다.',
    )
  record.jobId = `job_${Math.random().toString(16).slice(2, 8)}`
  record.status = 'PENDING'
  record.progress = 0
  record.stage = 'UPLOAD'
  record.message = '분석 재시작 요청이 접수되었습니다.'
  ...
  record.updatedAt = new Date().toISOString()
  await saveState()
  success(res, 200, { videoId: record.videoId, jobId: record.jobId, status: record.status })
  void advance(record)
})
...
app.post('/api/v1/videos/:id/analysis/cancel', async (req, res) => {
  ...
  if (record.status !== 'PENDING' && record.status !== 'PROCESSING')
    return failure(res, 409, 'INVALID_ANALYSIS_STATE', '진행 중인 분석만 취소할 수 있습니다.')
  record.status = 'CANCELLED'
  record.message = '분석이 취소되었습니다.'
  record.updatedAt = new Date().toISOString()
  await saveState()
  publish(record)
  success(res, 200, { videoId: record.videoId, jobId: record.jobId, status: record.status })
...
app.get('/api/v1/videos/:id/stream', async (req, res) => {
  ...
  try {
    ...
    if (!range) {
      ...
      return createReadStream(record.storagePath).pipe(res)
    }
    const match = /bytes=(\d*)-(\d*)/.exec(range)
    if (!match)
      return failure(res, 416, 'RANGE_NOT_SATISFIABLE', '요청한 영상 범위를 읽을 수 없습니다.')
    const start = match[1] ? Number(match[1]) : 0
    const end = match[2] ? Number(match[2]) : info.size - 1
    if (start >= info.size || end >= info.size || start > end)
      return failure(res, 416, 'RANGE_NOT_SATISFIABLE', '요청한 영상 범위를 읽을 수 없습니다.')
    res.status(206).setHeader('Content-Range', `bytes ${start}-${end}/${info.size}`)
    res.setHeader('Content-Length', end - start + 1)
    return createReadStream(record.storagePath, { start, end }).pipe(res)
  } catch {
    return failure(res, 404, 'VIDEO_NOT_FOUND', '영상 파일을 찾을 수 없습니다.')
  }
})
...
app.get('/api/v1/videos/:id/frames/:frameId', (req, res) => {
  ...
  const png = Buffer.from(
    'iVBORw0KGgoAAAANSUhEUgAAAEAAAAAQCAYAAACqaXHeAAAAC0lEQVR42mNkYPgPAAEDAQAIWqD2AAAAAElFTkSuQmCC',
    'base64',
  )
...
wss.on('connection', socket => {
  ...
  socket.on('message', raw => {
    ...
    if (frame.command === 'SUBSCRIBE' && frame.headers.destination)
      subscriptions.get(socket)?.set(frame.headers.destination, frame.headers.id ?? 'sub-0')
    if (frame.command === 'UNSUBSCRIBE' && frame.headers.id) {
      const topics = subscriptions.get(socket)
      if (topics)
        for (const [topic, id] of topics) if (id === frame.headers.id) topics.delete(topic)
    }
  })
...
for (const record of Object.values(state.videos))
  if (record.status === 'PENDING' || record.status === 'PROCESSING') void advance(record)
httpServer.listen(port, () =>
  console.log(`Creator Risk mock server listening on http://localhost:${port}`),
)
