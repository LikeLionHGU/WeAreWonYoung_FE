...
export default function EventDetailCard({
  ...
  return (
    <article ref={cardRef} className="report-selected-card">
      <div className="report-card-top">
        ...
        <div className="report-card-position">
          <span className="report-scrubber-mini">
            <i style={{ width: `${mediaDuration ? Math.min(100, Math.max(4, selected.startMs / 1000 / mediaDuration * 100)) : 0}%` }} />
          </span>
          <span>{formattedDuration} 중 {formatTime(selected.startMs)}</span>
          <span>{selectedIndex + 1} / {totalCount}</span>
        </div>
      </div>

      <h2>{reportEventTitle(selected)}</h2>
      <div className="report-reason"><h3>왜 확인하나요?</h3><p>{selected.reason}</p></div>
      ...
      {selected.type === 'SPEECH' && selected.candidateType === 'SPEECH_REVIEW' && (selected.contextBefore || selected.contextAfter) && (
        ...
        {selectedReferences.length > 0 ? (
          <div className="report-reference-items">
            {selectedReferences.map(reference => (
              <article className="report-reference-item" key={`${reference.url}-${reference.title}`}>
                <div>
