  ...
  if (!report && !error)
    return (
      <main className="center-page">
        <Loading label="검수 완료 화면을 준비하는 중" />
      </main>
    )
  if (error)
    return (
  ...
  return (
    <main className="completion-page">
      ...
      <section className="completion-summary" aria-label="검수 요약">
        <div>
          <span>수정함</span>
          <strong>{edited.length}건</strong>
        </div>
        <div>
          <span>유지함</span>
          <strong>{kept.length}건</strong>
        </div>
        <div>
          <span>보류</span>
          <strong>{held.length}건</strong>
        </div>
      </section>
        ...
        <div className="completion-list">
          {actionEvents.length > 0 ? (
            actionEvents.map(event => (
              <div className="completion-row" key={event.id}>
                <span className="completion-time">{formatTime(event.startMs)}</span>
                <span className="completion-kind">{reportEventKind(event)}</span>
                <p>{reportEventTitle(event)}</p>
                <strong>수정함</strong>
              </div>
            ))
          ) : (
            <div className="history-empty">
              <p>수정하기로 한 구간이 없습니다.</p>
            </div>
          )}
        </div>
      </section>
      <div className="completion-bottom">
        <div>
          <Link className="completion-primary" to="/upload">
            새 영상 올리기
          </Link>
          <Link className="completion-secondary" to={`/videos/${id}/report`}>
            리포트 다시 보기
          </Link>
        </div>
        <p>보류 {held.length}건은 리포트에 남아 있습니다.</p>
      </div>
    </main>
  )
}
